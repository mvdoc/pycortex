...
`Tractogram` (`tractogram.py`) is the odd one out: a `Dataview` that is not braindata at all, holding streamlines as a flat `(N, 3)` point array plus an `(M+1,)` offset table (the TRX convention), loaded with `Tractogram.from_trx` through the optional `trx-python` extra (lazy import — CI's oldest Python is too old for it). It is viewer-only: quickflat ignores it, `_write_hdf` raises, and `Package` refuses to build a viewer from a tractogram alone.
