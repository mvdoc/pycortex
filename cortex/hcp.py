...
def cifti_to_surface(data, left_indices, right_indices):
    ...
    if data.ndim == 1:
        full = np.full(N_VERTICES_FS_LR_32K, np.nan, dtype=out_dtype)
        full[left_indices] = data[:n_left]
        full[right_offset + right_indices] = data[n_left : n_left + n_right]
    else:
        full = np.full((data.shape[0], N_VERTICES_FS_LR_32K), np.nan, dtype=out_dtype)
        full[:, left_indices] = data[:, :n_left]
        full[:, right_offset + right_indices] = data[:, n_left : n_left + n_right]
    return full
...
def project_fslr_to_fsaverage(
    ...
    if data.shape[1] != N_VERTICES_FS_LR_32K:
        raise ValueError(
            f"Expected {N_VERTICES_FS_LR_32K} fs_LR 32k vertices, got {data.shape[1]}."
        )
    ...
    for ih, hemi in enumerate(("L", "R")):
        ...
        result[:, tgt_sl] = _project_hemi(matrix, data[:, src_sl], nanmean)
