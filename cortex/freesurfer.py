...
def autorecon(fs_subject, type="all", parallel=True, n_cores=None):
    ...
    times = {"all": "12 hours", "2": "6 hours", "cp": "8 hours", "wm": "4 hours"}
    if str(type) in times:
        resp = input(
            "recon-all will take approximately %s to run! Continue? " % times[str(type)]
        )
...
def flatten(fs_subject, hemi, patch, freesurfer_subject_dir=None, save_every=None):
    ...
    if resp.lower() in ("y", "yes"):
        ...
        outpath = get_paths(
            fs_subject, hemi, freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name=patch + ".flat")
        if save_every is None:
            save_every_str = ""
        else:
            save_every_str = " -w %d" % save_every
        cmd = "mris_flatten -O fiducial{save_every_str} {inpath} {outpath}".format(
            inpath=inpath, outpath=outpath, save_every_str=save_every_str
        )
        print("Calling: ")
        print(cmd)
        sp.check_call(shlex.split(cmd))
...
def import_subj(
    ...
    for fsname, name in info_fs2pycortex.items():
        ...
        np.savez(surfinfo_template.format(name=name), left=-lh, right=-rh)
...
def import_flat(
    ...
    surfs = os.path.join(
        database.default_filestore, cx_subject, "surfaces", "flat_{hemi}.gii"
    )
    ...
    for hemi in hemis:
        if flat_type in ["freesurfer", "blender"]:
            surf_path = (
                ...
                else (patch + ".flat.blender")
            )
            pts, polys, _ = get_surf(
                fs_subject,
                hemi,
                "patch",
                surf_path,
...
def _remove_disconnected_polys(polys):
    ...
    row = np.concatenate(
        [polys[:, 0], polys[:, 1], polys[:, 0], polys[:, 2], polys[:, 1], polys[:, 2]]
    )
    col = np.concatenate(
        [polys[:, 1], polys[:, 0], polys[:, 2], polys[:, 0], polys[:, 2], polys[:, 1]]
    )
    data = np.ones(len(col), dtype=bool)
    graph = sp.coo_matrix((data, (row, col)), shape=(n_points, n_points), dtype=bool)
...
def make_fiducial(fs_subject, freesurfer_subject_dir=None):
    ...
    for hemi in ["lh", "rh"]:
        spts, polys, _ = get_surf(
            fs_subject, hemi, "smoothwm", freesurfer_subject_dir=freesurfer_subject_dir
        )
        ppts, _, _ = get_surf(
            fs_subject, hemi, "pial", freesurfer_subject_dir=freesurfer_subject_dir
        )
        fname = get_paths(
            fs_subject, hemi, "surf", freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name="fiducial")
...
def parse_curv(filename):
    ...
    with open(filename, "rb") as fp:
        fp.seek(15)
        return (
            np.frombuffer(fp.read(), dtype=">f4")
            .byteswap()
            .view(np.dtype(">f4").newbyteorder("="))
        )
...
def parse_patch(filename):
    ...
    with open(filename, "rb") as fp:
        ...
        data = np.frombuffer(
            fp.read(), dtype=[("vert", ">i4"), ("x", ">f4"), ("y", ">f4"), ("z", ">f4")]
        )
...
def get_surf(
    subject, hemi, type, patch=None, flatten_step=None, freesurfer_subject_dir=None
):
    """Read freesurfer surface file"""
    if type == "patch":
        assert patch is not None
        surf_file = get_paths(
            subject, hemi, "surf", freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name="smoothwm")
    else:
        surf_file = get_paths(
            subject, hemi, "surf", freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name=type)
    ...
    if patch is not None:
        patch_file = get_paths(
            subject, hemi, "patch", freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name=patch)
    ...
    return (
        pts,
        polys,
        get_curv(subject, hemi, freesurfer_subject_dir=freesurfer_subject_dir),
    )


def _move_labels(
    subject, label, hemisphere=("lh", "rh"), fs_dir=None, src_subject="fsaverage"
):
    """subject is a freesurfer subject"""
    if fs_dir is None:
        fs_dir = os.environ["SUBJECTS_DIR"]
    for hemi in hemisphere:
        srclabel = os.path.join(
            fs_dir,
            src_subject,
            "label",
        ...
        if not os.path.exists(fs_sub_dir):
            raise ValueError(
                "Freesurfer subject directory for subject ({}) does not exist!".format(
                    fs_sub_dir
                )
            )
        cmd = (
            "mri_label2label --srcsubject {src_subject} --trgsubject {subject} "
        ...
        proc = sp.Popen(to_call, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.PIPE)
...
def _parse_labels(label_files, cx_subject):
    ...
    for fname in label_files:
        with open(fname) as fid:
            lines = fid.readlines()
            lines = [
                [float(xx.strip()) for xx in x.split(" ") if xx.strip()]
                for x in lines[2:]
            ]
            vals = np.array(lines)
            if "/lh." in fname:
...
def get_label(
    ...
    **kwargs,
    ...
    label_files = [
        os.path.join(fs_dir, fs_subject, "label", "{}.{}.label".format(h, label))
        for h in hemisphere
    ]
...
def mri_surf2surf(data, source_subj, target_subj, hemi, subjects_dir=None):
    ...
    cmd = _mri_surf2surf_command(
        source_subj, target_subj, tf_in.name, tf_out.name, hemi
    )
    ...
    if exit_code != 0:
        ...
        raise Exception(
            ("Exit code {exit_code} means that mri_surf2surf failed").format(
                exit_code=exit_code
            )
        )
...
def _read_sphere_reg(subject, hemi, subjects_dir=None):
    ...
    """
    surf_file = get_paths(
        subject, hemi, "surf", freesurfer_subject_dir=subjects_dir
    ).format(name="sphere.reg")
...
def _surf2surf_nnfr_matrix(src_sphere, trg_sphere):
    ...
    * **Reverse**: any source vertex that was *not* selected by some target in
    ...
    matrix = coo_matrix(
        (np.ones(len(rows)), (rows, cols)), shape=(n_trg, n_src)
    ).tocsr()
...
_SURF2SURF_LEGACY_KWARGS = frozenset(
    ("n_neighbors", "random_state", "n_test_images", "coef_threshold", "renormalize")
)


def get_mri_surf2surf_matrix(
    source_subj,
    hemi,
    surface_type=None,
    ...
    **kwargs,
    ...
    if unexpected:
        raise TypeError(
            "get_mri_surf2surf_matrix got unexpected keyword argument(s) {}".format(
                sorted(unexpected)
            )
        )
...
def get_curv(fs_subject, hemi, type="wm", freesurfer_subject_dir=None):
    ...
    if type == "wm":
        curv_file = get_paths(
            fs_subject, hemi, "curv", freesurfer_subject_dir=freesurfer_subject_dir
        ).format(name="")
    else:
        curv_file = get_paths(
            fs_subject, hemi, "curv", freesurfer_subject_dir=freesurfer_subject_dir
...
def show_surf(subject, hemi, type, patch=None, curv=True, freesurfer_subject_dir=None):
    ...
    warnings.warn(
        (
            "Deprecated and probably broken! Try `cortex.segment.show_surf()`\n"
    ...
    pts, polys, idx = get_surf(
        subject, hemi, type, patch, freesurfer_subject_dir=freesurfer_subject_dir
    )
    if curv:
    ...
    src = mlab.pipeline.triangular_mesh_source(
        pts[:, 0], pts[:, 1], pts[:, 2], polys, scalars=curv, figure=fig
    )
    ...
    surf.parent.scalar_lut_manager.set(
        lut_mode="RdBu", data_range=[-1, 1], use_default_range=False
    )
    ...
    def picker_callback(picker):
        if picker.actor in surf.actor.actors:
            npts = np.append(
                cursors.data.points.to_array(), [pts[picker.point_id]], axis=0
            )
...
def write_decimated(path, pts, polys):
    ...
    data = np.zeros(
        (len(dpts),), dtype=[("vert", ">i4"), ("x", ">f4"), ("y", ">f4"), ("z", ">f4")]
    )
...
class SpringLayout(object):
    ...
    def view_step(self):
        ...
        if self.figure is None:
            self.figure = mlab.triangular_mesh(
                self.pts[:, 0],
                self.pts[:, 1],
                self.pts[:, 2],
                self.polys,
        ...
        self.step()
        self.figure.mlab_source.set(
            x=self.pts[:, 0], y=self.pts[:, 1], z=self.pts[:, 2]
        )
...
def _get_upsample_neighbors(hemi, ico_order, freesurfer_subjects_dir=None):
    ...
    bundle_keys = getattr(
        _upsample_neighbors_bundle, "files", list(_upsample_neighbors_bundle)
    )
    if key in bundle_keys:
        return np.asarray(_upsample_neighbors_bundle[key])

    # 2. User cache.
    cache_dir = os.path.join(appdirs.user_cache_dir("pycortex"), "upsample_fsaverage")
    ...
    pts, _ = nibabel.freesurfer.read_geometry(
        os.path.join(freesurfer_subjects_dir, "fsaverage", "surf", f"{hemi}.sphere.reg")
    )
    _, neighbors = KDTree(pts[:n_ico]).query(pts[n_ico:], k=1)
    neighbors = neighbors.astype(np.uint16 if neighbors.max() < 65536 else np.int64)
    os.makedirs(cache_dir, exist_ok=True)
    np.save(cache_path, neighbors)
    return neighbors


def upsample_to_fsaverage(data, data_space="fsaverage6", freesurfer_subjects_dir=None):
    ...
    for hemi, dt in zip(hemis, data_hemi):
        neighbors = _get_upsample_neighbors(hemi, ico_order, freesurfer_subjects_dir)
        # Fill the extra fsaverage vertices from their nearest low-res neighbor.
        projected_data.append(np.concatenate([dt, dt[:, neighbors]], axis=-1))
